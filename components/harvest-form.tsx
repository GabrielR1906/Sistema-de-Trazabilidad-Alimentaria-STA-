...
export function HarvestForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Capa de Lógica procesa el guardado
    const validation = await TraceabilityLogic.saveHarvest(data)
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex gap-2">
            ...
            <Button type="button" variant="outline" onClick={() => setBatchId(TraceabilityLogic.generateBatchId())}><Sparkles className="h-4 w-4" /></Button>
          </div>
